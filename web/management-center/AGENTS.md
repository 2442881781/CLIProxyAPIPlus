...
This directory contains the React 19 + TypeScript + Vite management frontend bundled with the CLI Proxy API monorepo. It talks to the backend Management API under `/v0/management`.
...
- Treat backend contracts as the source of truth. Inspect the repository root (`../..`) before changing endpoint names, payloads, provider keys, OAuth callback parameters, auth-file semantics, or plugin/config contracts. Do not modify the backend unless requested.
