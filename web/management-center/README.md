...
This customized frontend is maintained in `web/management-center` of the CLIProxyAPI monorepo. The repository root owns backend compatibility, deployment, and the generated `static/management.html` artifact.
...
- This directory is the Web UI portion of the monorepo. It talks to the CLI Proxy API **Management API** (`/v0/management`) to read/update config, upload credentials, and view logs.
...
- From the repository root, run `./scripts/build-management-center.sh` to verify the frontend and copy the artifact to `static/management.html`.
...
- The repository-root Dockerfile and `scripts/build-management-center.sh` build this directory and bundle `dist/index.html` as `static/management.html`.
