...
describe('isQuotaCacheFresh', () => {
  const now = 1_000_000;

  test('accepts recent successful snapshots only', () => {
    expect(isQuotaCacheFresh({ status: 'success', fetchedAt: now - 60_000 }, now)).toBe(true);
    expect(isQuotaCacheFresh({ status: 'success', fetchedAt: now - 5 * 60_000 }, now)).toBe(false);
    expect(isQuotaCacheFresh({ status: 'success' }, now)).toBe(false);
    expect(isQuotaCacheFresh({ status: 'error', fetchedAt: now - 1_000 }, now)).toBe(false);
  });

  test('rejects future timestamps instead of trusting a skewed cache', () => {
    expect(isQuotaCacheFresh({ status: 'success', fetchedAt: now + 1 }, now)).toBe(false);
  });
});
