...
此定制前端现在由 CLIProxyAPI 主仓库的 `web/management-center` 目录统一维护。后端兼容性、部署流程以及生成的 `static/management.html` 均由主仓库负责。
...
- 本目录是主仓库中的 Web 管理界面，通过 CLI Proxy API 的 **Management API**（`/v0/management`）读取/修改配置、上传凭据与查看日志。
- 它 **不是** 代理本体，不参与流量转发。
...
- 从主仓库根目录执行 `./scripts/build-management-center.sh`，会构建并复制为 `static/management.html`。
...
- 主仓库 Dockerfile 和 `scripts/build-management-center.sh` 会构建本目录，并将 `dist/index.html` 打包为 `static/management.html`。
