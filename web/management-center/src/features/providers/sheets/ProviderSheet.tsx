...
export function ProviderSheet({
  ...
  const providerDisplayName =
    ...
    state.resource?.name?.toLowerCase().includes('zhipu glm')
      ? t('providersPage.providerNames.zhipu')
      : t(`providersPage.providerNames.${state.brand}`);

  const titleText =
    state.mode === 'create'
      ? `${t('providersPage.form.createEyebrow')} · ${providerDisplayName}`
      : state.mode === 'edit'
        ? `${t('providersPage.form.editEyebrow')} · ${providerDisplayName}`
        : `${t('providersPage.detail.title')} · ${providerDisplayName}`;
