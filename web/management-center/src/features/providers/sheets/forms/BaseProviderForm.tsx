...
/** Module-level constant so the picker receives a stable array reference. */
const DISABLE_ALL_RULES = [DISABLE_ALL_RULE];

type HeaderInput = ProviderEntryFormInput['headers'][number];
...
const ZHIPU_TEAM_ORGANIZATION_HEADER = 'bigmodel-organization';
const ZHIPU_TEAM_PROJECT_HEADER = 'bigmodel-project';

const readHeaderValue = (headers: HeaderInput[], name: string): string =>
  headers.find((entry) => entry.key.trim().toLowerCase() === name)?.value ?? '';

const writeHeaderValue = (headers: HeaderInput[], name: string, value: string): HeaderInput[] => {
  const existingIndex = headers.findIndex((entry) => entry.key.trim().toLowerCase() === name);
  if (existingIndex >= 0) {
    if (!value) return headers.filter((_, index) => index !== existingIndex);
    return headers.map((entry, index) => (index === existingIndex ? { ...entry, value } : entry));
  }
  return value
    ? [...headers.filter((entry) => entry.key || entry.value), { key: name, value }]
    : headers;
};
...
export function BaseProviderForm({
  ...
  const fid = useId();
  const [form, setForm] = useState<ProviderEntryFormInput>(() => buildInitialForm(brand, resource));
  const [initialFormSignature] = useState<string>(() =>
    JSON.stringify(buildInitialForm(brand, resource))
  );
   ...
   *
   * Both may be empty: `vertex` supports model exclusions but is not in MODEL_DISCOVERY_BRANDS,
   * so it never has discovery results; other brands have none before the user runs discovery.
   * An empty catalog is therefore normal, and the picker must fall back to a rules-only editor.
   */
  ...
  const excludedCatalogState: ExcludedModelsCatalogState = discovery.loading
    ...
    : discovery.error
      ...
      : excludedCandidates.length === 0
        ? 'unavailable'
        : 'ready';
  const isZhipuCodingPlanForm =
    brand === 'openaiCompatibility' &&
    /(?:open\.bigmodel\.cn|api\.z\.ai)\/api\/coding\//i.test(form.baseUrl);
  const zhipuTeamOrganization = readHeaderValue(form.headers, ZHIPU_TEAM_ORGANIZATION_HEADER);
  const zhipuTeamProject = readHeaderValue(form.headers, ZHIPU_TEAM_PROJECT_HEADER);
  const updateZhipuTeamHeader = (name: string, value: string) => {
    updateField('headers', writeHeaderValue(form.headers, name, value));
  };
  ...
  return (
    <form id={formId} className={styles.form} onSubmit={handleSubmit} noValidate>
      ...
      <div className={styles.section}>
        ...
        {isZhipuCodingPlanForm ? (
          <div className={styles.zhipuTeamFields}>
            <div className={styles.fieldRow}>
              <div className={styles.field}>
                <label className={styles.label} htmlFor={`${fid}-zhipu-team-organization`}>
                  {t('providersPage.form.zhipuTeamOrganization')}
                </label>
                <input
                  id={`${fid}-zhipu-team-organization`}
                  className={styles.input}
                  value={zhipuTeamOrganization}
                  onChange={(event) =>
                    updateZhipuTeamHeader(ZHIPU_TEAM_ORGANIZATION_HEADER, event.target.value)
                  }
                  placeholder="Bigmodel-Organization"
                  disabled={mutating}
                />
              </div>
              <div className={styles.field}>
                <label className={styles.label} htmlFor={`${fid}-zhipu-team-project`}>
                  {t('providersPage.form.zhipuTeamProject')}
                </label>
                <input
                  id={`${fid}-zhipu-team-project`}
                  className={styles.input}
                  value={zhipuTeamProject}
                  onChange={(event) =>
                    updateZhipuTeamHeader(ZHIPU_TEAM_PROJECT_HEADER, event.target.value)
                  }
                  placeholder="Bigmodel-Project"
                  disabled={mutating}
                />
              </div>
            </div>
            <span className={styles.labelHint}>{t('providersPage.form.zhipuTeamIdsHint')}</span>
          </div>
        ) : null}
        ...
        <Collapsible label={t('providersPage.form.excludedSection')}>
          <div className={styles.field}>
            <ExcludedModelsPicker
              ...
              disabled={mutating}
              // `'*'` disables this provider and is owned exclusively by the Disabled toggle below.
              // The picker filters it both ways and explains why manually entering `*` is rejected.
