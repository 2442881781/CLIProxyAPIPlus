/**
 * Batch quota loading across mixed providers.
 *
 * Three guards prevent duplicate or stale commits:
 * - loadingRef deduplicates concurrent batches;
 * - requestIdRef discards superseded responses;
 * - cacheGeneration prevents an old session from writing into a new one.
 * Results commit per provider so fast providers do not wait for slow ones.
 */
                ...
                results.forEach((result) => {
                  nextState[result.name] =
                    result.status === 'success'
                      ? { ...adapter.buildSuccessState(result.data), fetchedAt: Date.now() }
