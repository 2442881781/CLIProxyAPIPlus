/**
 * Quota page with provider tabs and a unified card grid.
 *
 * Preserved behavior contracts:
 * - Fresh successful snapshots survive reloads; missing or stale cards load automatically.
 * - cacheGeneration isolates sessions and useQuotaBatchLoader deduplicates requests.
 * - Provider caches are pruned after the credential list changes.
 * - This page exclusively owns the single useHeaderRefresh slot.
 */

import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { authFilesApi, pluginsApi, providersApi } from '@/services/api';
...
import { buildAIProviderQuotaFiles } from './providerQuotaSources';
...
/** Timeline lane names match card titles; module scope keeps the memo dependency stable. */
...
export function QuotaPage() {
  ...
  // Stagger the title, metadata, actions, and tabs by 70 ms.
  ...
  const loadFiles = useCallback(async () => {
    ...
    setError('');
    try {
      const [authResponse, commandCodeConfig, openCodeConfig, openAIProviders] = await Promise.all([
        authFilesApi.list(),
        pluginsApi.getConfig('commandcode').catch(() => ({})),
        pluginsApi.getConfig('opencode-go-cliproxyapi').catch(() => ({})),
  ...
  /* ---------- Quota cache ----------
   * Must precede classification because recovery sorting reads this state. */
  ...
  // Subscribe to the minute clock only for recovery sorting. Otherwise pageItems would
  // change identity every minute and repeatedly trigger the refresh-all settling effect.
  ...
  // Sort before pagination so recovery priority applies globally, not per page.
  ...
  // Keep only credentials that still exist after the list settles.
  useEffect(() => {
    if (loading) return;
  ...
  // Refresh the credential list first, then load current-page quotas after it settles.
  ...
  // Restore fresh persisted snapshots immediately and query only missing or stale cards once.
  useEffect(() => {
    if (loading || disableControls || batchLoading) return;

    const now = Date.now();
    const targets = pageItems.filter((entry) => {
      const key = `${entry.type}:${entry.file.name}`;
      if (isQuotaCacheFresh(getQuota(entry), now)) return false;
      if (autoLoadAttemptedRef.current.has(key)) return false;
      autoLoadAttemptedRef.current.add(key);
      return true;
    });

    if (targets.length > 0) {
      void loadQuota(targets);
    }
  }, [batchLoading, disableControls, getQuota, loadQuota, loading, pageItems]);

  const canUseActions = !disableControls && !loading;

  /* ---------- One-time initial card stagger ----------
   * Mounted cards capture their delay before cardsAnimated flips. Later tab, page, and
   * refresh remounts receive null so the entrance animation does not replay. */
  ...
  return (
    <div className={styles.page} ref={revealRef}>
      ...
      <section className={styles.workbench}>
        {/* Tabs and sorting enter together because useRevealGroup staggers data-reveal descendants. */}
          ...
          <div className={styles.grid}>
            {pageItems.map((entry, index) => (
              <QuotaCard
                ...
                onReset={(action) => resetQuota(entry.file, QUOTA_ADAPTERS[entry.type], action)}
              />
            ))}
          </div>
        )}
        ...
        {/* Limit timeline lanes to the current page to keep large credential sets bounded. */}
        <QuotaTimeline
          entries={pageItems}
          quotaFor={getQuota}
