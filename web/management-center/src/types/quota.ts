...
export interface QuotaCacheMetadata {
  /** Epoch milliseconds when the last successful upstream query completed. */
  fetchedAt?: number;
}
...
export interface ClaudeQuotaState extends QuotaCacheMetadata {
...
export interface AntigravityQuotaState extends QuotaCacheMetadata {
  status: 'idle' | 'loading' | 'success' | 'error';
...
export interface CodexQuotaState extends QuotaCacheMetadata {
  status: 'idle' | 'loading' | 'success' | 'error';
...
export interface KimiQuotaState extends QuotaCacheMetadata {
  status: 'idle' | 'loading' | 'success' | 'error';
  rows: KimiQuotaRow[];
  error?: string;
  errorStatus?: number;
...
export interface XaiQuotaState extends QuotaCacheMetadata {
  status: 'idle' | 'loading' | 'success' | 'error';
...
export type ProviderQuotaResetType = 'FIVE_HOUR' | 'WEEK';

export interface ProviderQuotaResetCredit {
  actionId: string;
...
export interface ProviderQuotaData {
  ...
  balance?: number;
  balanceUnit?: string;
  resetCredits?: ProviderQuotaResetCredit[];
  resetCreditsError?: string;
}

export interface ProviderQuotaState extends ProviderQuotaData, QuotaCacheMetadata {
  status: 'idle' | 'loading' | 'success' | 'error';
  error?: string;
  errorStatus?: number;
}
