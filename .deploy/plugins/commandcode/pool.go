...
// pool retains transport helpers for compatibility tests. Live executor paths
// use the host HTTP client associated with the one auth selected by CPA.
type pool struct {
	mu      sync.Mutex
	rnd     *rand.Rand
...
// members exposes configured credentials for compatibility tests and tooling.
// Live execution deliberately does not call this method.
