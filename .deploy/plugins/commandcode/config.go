...
type pluginConfig struct {
	...
	// APIKeys declares credentials that are expanded into independently
	// schedulable host auths. When non-empty it wins over APIKey.
...
// APIKeyEntry is one host-managed credential.
type APIKeyEntry struct {
	Key      string `yaml:"key" json:"api_key"`
...
func (en APIKeyEntry) attributes() map[string]string {
	attrs := map[string]string{
		"api_key":   strings.TrimSpace(en.Key),
		"auth_kind": "apikey",
		"weight":    strconv.Itoa(en.normWeight()),
	}
	if en.Priority != 0 {
		attrs["priority"] = strconv.Itoa(en.Priority)
	}
	return attrs
}

func (c *pluginConfig) credentialEntries() []APIKeyEntry {
	if c == nil {
		return nil
	}
	if len(c.APIKeys) > 0 {
		entries := make([]APIKeyEntry, 0, len(c.APIKeys))
	...
	if key := strings.TrimSpace(c.APIKey); key != "" {
		return []APIKeyEntry{{Key: key, Weight: 1}}
	}
	return nil
}

func selectedCredential(req pluginapi.ExecutorRequest) (APIKeyEntry, bool) {
	entry := APIKeyEntry{Key: strings.TrimSpace(req.AuthAttributes["api_key"]), Weight: 1}
	if entry.Key == "" && req.AuthMetadata != nil {
		if key, ok := req.AuthMetadata["api_key"].(string); ok {
