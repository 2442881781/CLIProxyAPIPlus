...
// capabilities this plugin declares: model provider/router, auth provider,
// executor, and request/response translators.
package main
...
func handleRegister(request []byte) ([]byte, error) {
	...
	built, p := plug.BuildForHost(req.ConfigYAML, req.SchemaVersion, callHostRaw)
	if p == nil {
...
func callHostRaw(method string, request []byte) ([]byte, error) {
	...
	code := C.commandcode_call_host(host, cMethod, requestPtr, C.size_t(len(request)), &resp)
	if resp.ptr != nil {
