...
func Load(yamlBytes []byte) (Config, error) {
	...
	for i, k := range raw.APIKeys {
		keys[i] = APIKey{
			Value: os.ExpandEnv(k.Value), Label: strings.TrimSpace(k.Label), Priority: k.Priority,
			Weight: k.Weight, ProxyURL: strings.TrimSpace(k.ProxyURL), Disabled: k.Disabled,
		}
	}
...
func (c Config) validate() error {
	...
	for i, k := range c.APIKeys {
		...
		if k.Weight < 0 {
			return fmt.Errorf("api-keys[%d].weight: must not be negative", i)
		}
		if k.ProxyURL != "" {
			if err := validateURL(fmt.Sprintf("api-keys[%d].proxy_url", i), k.ProxyURL, true); err != nil {
				return err
			}
		}
	}
