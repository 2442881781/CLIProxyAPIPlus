...
// AuthDelete removes one plugin-owned physical auth record.
func (b *HostBridge) AuthDelete(ctx context.Context, name string) error {
	env, err := b.invoke(ctx, "host.auth.delete", struct {
		Name string `json:"name"`
	}{Name: name}, "host auth delete")
	if err != nil {
		return err
	}
	if !env.OK {
...
func (b *HostBridge) Do(ctx context.Context, req pluginapi.HTTPRequest) (pluginapi.HTTPResponse, error) {
	return b.DoWithCallbackID(ctx, "", req)
}

// DoWithCallbackID issues a host HTTP request in the callback scope created by
// the host for quota/executor RPCs.
func (b *HostBridge) DoWithCallbackID(ctx context.Context, callbackID string, req pluginapi.HTTPRequest) (pluginapi.HTTPResponse, error) {
...
func (b *HostBridge) DoStream(ctx context.Context, req pluginapi.HTTPRequest) (statusCode int, headers http.Header, upstreamStreamID string, err error) {
	return b.DoStreamWithCallbackID(ctx, "", req)
}

// DoStreamWithCallbackID opens an upstream stream in the host callback scope.
func (b *HostBridge) DoStreamWithCallbackID(ctx context.Context, callbackID string, req pluginapi.HTTPRequest) (statusCode int, headers http.Header, upstreamStreamID string, err error) {
	payloadBytes, _ := json.Marshal(hostHTTPReq{
		HostCallbackID: callbackID, Method: req.Method, URL: req.URL, Headers: req.Headers, Body: req.Body,
	})
