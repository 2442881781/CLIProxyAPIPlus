...
func TestAuthProviderParseAndRefresh(t *testing.T) {
	...
	if err != nil || !parsed.Handled || parsed.Auth.ID != "stable" || parsed.Auth.Label != "Primary" || parsed.Auth.Attributes["api_key"] != secret || !strings.Contains(string(parsed.Auth.StorageJSON), `"id":"stable"`) {
...
func TestAuthProviderAliasesAndFilenameFallback(t *testing.T) {
	...
	if err != nil || !parsed.Handled || !strings.HasPrefix(parsed.Auth.ID, "opencode-go-key-") || parsed.Auth.Attributes["api_key"] != "sk-alias-key" {
