...
func (f *fakeCaller) call(method string, payload []byte) ([]byte, error) {
	...
	if method == "host.auth.delete" && err == nil && hostEnvelopeOK(raw) {
		var req struct {
			Name string `json:"name"`
		}
		if json.Unmarshal(payload, &req) == nil {
			f.mu.Lock()
			delete(f.authFiles, req.Name)
			f.mu.Unlock()
		}
	}
	return raw, err
...
func (f *fakeCaller) authListResponse() []byte {
	...
	for name := range f.authFiles {
		files = append(files, pluginapi.HostAuthFileEntry{
			ID: strings.TrimSuffix(name, ".json"), Name: name, Type: ProviderID, Provider: ProviderID, Source: "file", Path: name,
		})
	}
	f.mu.Unlock()
...
func lifecycleRequestBody(yamlText string) []byte {
	b, _ := json.Marshal(lifecycleRequest{ConfigYAML: []byte(yamlText), SchemaVersion: pluginSchemaVersionHostAuthDelete})
	return b
}
...
func TestLifecycleMaterializesDeterministicAuthRecords(t *testing.T) {
	...
	if len(calls) != 4 {
		t.Fatalf("unchanged reconfigure auth saves = %d, want 4 (two idempotent upserts)", len(calls))
	}
	...
	for _, call := range calls {
		...
		if record.Type != ProviderID || record.ID != "opencode-go-key-"+wantHash || record.Label != "OpenCode Go credential "+wantHash[:12] || wire.Name != record.ID+".json" {
	...
	if len(f.callsOf(pluginabi.MethodHostAuthSave)) != 5 {
		t.Fatalf("removed-key reconfigure save count = %d, want 5", len(f.callsOf(pluginabi.MethodHostAuthSave)))
	}
	if len(f.callsOf("host.auth.delete")) != 1 {
		t.Fatalf("removed-key delete calls = %d, want 1", len(f.callsOf("host.auth.delete")))
	}
	if _, err := m.HandleCall("plugin.reconfigure", lifecycleRequestBody("api-keys:\n  - value: "+first+"\n  - value: "+third+"\n")); err != nil {
	...
	calls = f.callsOf(pluginabi.MethodHostAuthSave)
	if len(calls) != 7 {
		t.Fatalf("new-key reconfigure auth saves = %d, want 7", len(calls))
	}
	var newRecord struct {
		APIKey string `json:"api_key"`
	}
	var wire pluginapi.HostAuthSaveRequest
	if err := json.Unmarshal(calls[len(calls)-1].payload, &wire); err != nil {
...
func TestLifecycleUsesCPAAuthListAfterManagerRestart(t *testing.T) {
	...
	if got := len(f.callsOf(pluginabi.MethodHostAuthSave)); got != 2 {
		t.Fatalf("auth saves after manager restart = %d, want 2 idempotent upserts", got)
	}
}
...
func TestOverlappingLifecyclesLeaveSingleTicker(t *testing.T) {
	...
	wg.Wait()
	if got := len(f.callsOf(pluginabi.MethodHostAuthSave)); got != 2 {
		t.Fatalf("overlapping lifecycle auth saves = %d, want 2 serialized idempotent upserts", got)
	}
	done := m.closeStop()
	if done == nil {
