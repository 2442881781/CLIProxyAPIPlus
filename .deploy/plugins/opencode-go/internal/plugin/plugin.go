...
func (m *Manager) HandleCall(method string, request []byte) (resp []byte, err error) {
	...
	case pluginabi.MethodQuotaDescribe:
		resp, err := (standardQuotaProvider{}).DescribeQuota(context.Background(), pluginapi.QuotaDescribeRequest{})
		if err != nil {
	...
	case pluginabi.MethodQuotaFetch:
		...
		if json.Unmarshal(request, &req) != nil {
			return ErrEnvelope("invalid_request", "malformed quota request body"), nil
		}
		quotaReq := req.QuotaFetchRequest
		quotaReq.HTTPClient = quotaBridgeClient{bridge: m.bridge, callbackID: req.HostCallbackID}
		resp, err := (standardQuotaProvider{manager: m}).FetchQuota(context.Background(), quotaReq)
		if err != nil {
			return ErrEnvelope("quota_failure", err.Error()), nil
		}
		return okEnvelope(resp), nil
	case pluginabi.MethodQuotaReset:
		resp, _ := (standardQuotaProvider{}).ResetQuota(context.Background(), pluginapi.QuotaResetRequest{})
		return okEnvelope(resp), nil
...
func registrationEnvelope(schemaVersion uint32) []byte {
	if schemaVersion == 0 || schemaVersion > pluginabi.SchemaVersion {
...
func (m *Manager) handleLifecycle(request []byte) ([]byte, error) {
	...
	defer cancel()
	if err := m.materializeAuthRecords(ctx, cfg, req.SchemaVersion >= pluginSchemaVersionHostAuthDelete); err != nil {
	...
	return registrationEnvelope(req.SchemaVersion), nil
}

// materializeAuthRecords converges plugin-managed physical auth files to the
// configured key set. Stable hashed names never expose credential material.
func (m *Manager) materializeAuthRecords(ctx context.Context, cfg config.Config, allowDelete bool) error {
	if m.bridge == nil {
		return nil
	}
	...
	for _, entry := range entries {
		if name := strings.TrimSpace(entry.Name); name != "" {
			...
			if strings.HasPrefix(name, "opencode-go-key-") && strings.HasSuffix(strings.ToLower(name), ".json") &&
				(strings.EqualFold(entry.Provider, ProviderID) || strings.EqualFold(entry.Type, ProviderID)) {
				managed[name] = struct{}{}
			}
		}
	...
	desired := make(map[string]struct{}, len(cfg.APIKeys))
	for _, key := range cfg.APIKeys {
		id, label := credentialIdentity(key.Value)
		if custom := strings.TrimSpace(key.Label); custom != "" {
			label = custom
		}
		name := id + ".json"
		desired[name] = struct{}{}
		...
		}{ProviderID, id, label, key.Value, key.Priority, weight, key.ProxyURL, key.Disabled})
		if errMarshal != nil {
	...
	for name := range managed {
		...
		if errDelete := m.bridge.AuthDelete(ctx, name); errDelete != nil {
			return errDelete
		}
	}
	return nil
