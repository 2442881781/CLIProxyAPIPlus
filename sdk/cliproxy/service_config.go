...
func newRoutingSelector(state routingRuntimeState) coreauth.Selector {
	...
	if state.sessionAffinity {
		...
		selector = coreauth.NewSessionAffinitySelectorWithConfig(coreauth.SessionAffinityConfig{
			...
			QuotaAware:       state.quotaAware,
		})
	} else if state.quotaAware {
		selector = coreauth.NewQuotaAwareSelector(selector)
	}
	return selector
}
...
func (s *Service) applyConfigRuntime(ctx context.Context, commit configCommit, synthesizeConfigAuths bool) bool {
	...
	if errUsagePersistence := redisqueue.ConfigureUsageMonitorPersistence(cfg.AuthDir); errUsagePersistence != nil {
		log.Warnf("failed to update usage monitor persistence: %v", errUsagePersistence)
	}
